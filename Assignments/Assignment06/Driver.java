package OS&Networking.Assignments.Assignment06;

public class Driver {

}
// This assignment is detailed at the end of Chapter 9 of your OS textbook
// beginning on page 432 (459 in the full text). The problem is titled:
// Designing a Virtual Memory Manager. Follow the instructions provided in the
// book, but complete the project in Java rather than in C.
/*
 ...
 * size 216 = 65,536 bytes. Your program will read from a file containing logical addresses and, using a TLB as well
 * as a page table, will translate each logical address to its corresponding physical address and output the value of 
 * the byte stored at the translated physical address. The goal behind this project is to simulate the steps involved 
 * in translating logical to physical addresses.
 * 
 * Specifics
 ...
 * These 16 bits are divided into (1) an 8-bit page number and (2) 8-bit page
 * offset.
 * Hence, the addresses are structured as shown in Figure 8.33.
 ...
 * page
 * number
 * Figure 8.33 Address structure.
 * Address Translation
 * Your program will translate logical to physical addresses using a TLB and
 * page
 * table as outlined in Section 7.5. First, the page number is extracted from
 * the
 * logical address, and the TLB is consulted. In the case of a TLB-hit, the
 * frame
 ...
 * address-translation process appears in Figure 8.34.
 * Handling Page Faults
 * Your program will implement demand paging as described in Section 8.2. The
 * backing store is represented by the file BACKING STORE.bin, a binary file of
 * size 65,536 bytes.When a page fault occurs, you will read in a 256-byte page from
 * the file BACKING STORE and store it in an available page frame in physical memory.
 * For example, if a logical address with page number 15 resulted in a page fault,
 * your program would read in page 15 from BACKING STORE (remember that pages begin at 0 and are 256 bytes in size) 
 * and store it in a page frame in physical memory. Once this frame is stored (and the page table and TLB are
 * updated), subsequent accesses to page 15 will be resolved by either the TLB or the page table.
 * You will need to treat BACKING STORE.bin as a random-access file so that
 * you can randomly seek to certain positions of the file for reading. We suggest using the standard C library 
 * functions for performing I/O, including fopen(), fread(), fseek(), and fclose().
 ...
 * space). Your program will open this file, read each logical address and
 * translate
 * it to its corresponding physical address, and output the value of the signed
 * byte
 * at the physical address.
 * How to Begin
 * First, write a simple program that extracts the page number and offset (based on Figure 8.33) 
 ...
 * You can integrate the TLB once your page table is working properly. Remember,
 * address translation can work without a TLB; the TLB just makes it faster.
 * When you are ready to implement the TLB, recall that it has only 16 entries, so
 * you will need to use a replacement strategy when you update a full TLB. You may
 ...
 * stored at the correct physical address. (Recall that in the C language, the
 * char
 ...
 * values for the file addresses.txt. You should use this file to determine if
 * your
 ...
 * and do not reflect any memory access locality, do not expect to have a high
 * TLB
 * hit rate.
 * Modifications
